...
- Keep tool schemas in `src/schemas/schemas.ts` using Zod `.strict()` objects. Don't use `.refine()` at the top level, the MCP SDK needs direct access to `.shape`, so put cross-field validation in the tool handler instead.
...
Open an issue with as much detail as you can: Node version, OS, and the exact error or output if applicable.
